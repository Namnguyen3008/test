        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if '429' in err_msg or 'quota' in err_msg.lower() or 'resource' in err_msg.lower():
                    ...
                    time.sleep(1)
        raise RuntimeError(f'Failed batch embedding after {max_retries} attempts')

    def run(self):
        print('=== VMEC-01 POSTGRESQL + PGVECTOR MULTI-KEY BATCH IMPORTER ===')
        print(f'🔑 Loaded {len(self.keys)} API Keys ({self.total_workers} Worker Endpoints)')
        
        if not os.path.exists(CATALOG_DB):
            print(f'❌ SQLite catalog not found at {CATALOG_DB}')
            return
        ...
        total_rows = len(rows)
        print(f'📊 Found {total_rows:,} total rows in catalog database')
        ...
        print(f'▶️ Resuming import from Index {start_idx:,} / {total_rows:,}')

        rec_sql = text('INSERT INTO knowledge_records (id, release_id, mode, source_row_key, specialty_id, content_hash) VALUES (:id, 'production-v1', 'production', :r_key, :s_id, :c_hash) ON CONFLICT (id) DO NOTHING')
        chunk_sql = text('INSERT INTO knowledge_chunks (id, record_id, chunk_index, chunk_text, token_count, content_hash) VALUES (:id, :rec_id, 0, :txt, :token_cnt, :c_hash) ON CONFLICT (id) DO NOTHING')
        ...
        for i in range(start_idx, total_rows, BATCH_SIZE):
            ...
            with self.engine.begin() as conn:
                for idx_in_batch, (vec, (r_key, s_id, txt, c_hash)) in enumerate(zip(embeddings, metadata_list)):
                    rec_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f'{r_key}:{c_hash}'))
                    chunk_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f'{rec_id}:chunk:0'))

                    conn.execute(rec_sql, {'id': rec_id, 'r_key': r_key, 's_id': s_id, 'c_hash': c_hash})
                    conn.execute(chunk_sql, {'id': chunk_id, 'rec_id': rec_id, 'txt': txt, 'token_cnt': len(txt.split()), 'c_hash': c_hash})
                    conn.execute(emb_sql, {
                        'chunk_id': chunk_id,
                        'model_id': model_id,
                        'embedding': str(vec),
                        'c_hash': c_hash
            ...
            eta_mins = ((total_rows - current_end_idx) / rate) / 60 if rate > 0 else 0

            print(f'✅ [BATCH {current_end_idx // BATCH_SIZE:,} / {total_rows // BATCH_SIZE + 1:,}] '
                  f'Processed {current_end_idx:,}/{total_rows:,} ({current_end_idx/total_rows*100:.1f}%) | '
                  f'Key #{key_num} | Model: {model_id} | Rate: {rate*60:.0f} rec/min | ETA: {eta_mins:.1f}m')

            time.sleep(1.5)

        print(f'🎉 IMPORT COMPLETE! Successfully processed {total_processed:,} records into PostgreSQL pgvector!')
