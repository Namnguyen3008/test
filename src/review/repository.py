...
class ReviewRepository:
        ...
        with self.session.begin():
            ...
            if existing:
                if existing.content_hash != content_hash:
                    raise ReviewConflictError("Review origin was reused with different content")
                return existing
            now = self.clock()
    ...
    def queue(self) -> list[ClinicalReviewItem]:
        return list(
            self.session.scalars(
                select(ClinicalReviewItem).order_by(ClinicalReviewItem.updated_at, ClinicalReviewItem.id)
            )
        )
    ...
    def safe_export(self, release_id: str) -> dict[str, object]:
        ...
        decisions = (
            list(
                self.session.scalars(
                    select(ClinicalReviewDecision).where(
                        ClinicalReviewDecision.item_id.in_([item.id for item in items])
                    )
                )
            )
            if items
